...
func deployDatabaseService(deployService *service.DeployService, dbService domain.Service) error {
	dbConfig, err := database.GetServiceByName(dbService.ImageName)
	if err != nil {
		return fmt.Errorf("error getting database service configuration: %w", err)
	}

	if dbConfig.Name == "" {
		return fmt.Errorf("database service %s not found", dbService.ImageName)
	}

	containerName := fmt.Sprintf("%s-db", strings.ToLower(dbConfig.Name))

	// Check if the service is already running
	isRunning, err := deployService.DockerAdapter.IsServiceRunning(containerName)
	if err != nil {
		return fmt.Errorf("error checking if database service %s is running: %w", dbConfig.Name, err)
	}
	...
	deployService.DockerAdapter.RunServiceWithDeploy(dbConfig, containerName)
